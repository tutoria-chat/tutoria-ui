  ...
  const sections = useMemo((): Section[] => {
    ...
    if (roleGuide === 'superAdmin') {
      return [
        ...
        { id: 'globalSearch', title: t('superAdmin.globalSearch.title'), icon: Search },
  ...
  const renderSectionDetails = (sectionKey: string, sectionId: string) => {
          ...
          {t.has(`${sectionKey}.responsibilities`) && (
            <>
              ...
              <ul className="space-y-3">
                {(t.raw(`${sectionKey}.responsibilities`) as string[]).map((responsibility, idx) => (
    ...
    // Workflow section - render numbered steps from workflow.steps array
      ...
      if (t.has(`${sectionKey}.steps`)) {
        const steps = t.raw(`${sectionKey}.steps`) as Array<{ number: number; title: string; description: string }>;
        content.push(
    ...
    // Tutorial sections (creatingCourses, creatingModules, etc.)
      ...
      if (t.has(`${sectionKey}.steps`)) {
        const steps = t.raw(`${sectionKey}.steps`) as Array<{ number: number; title: string; desc: string }>;
        content.push(
      ...
      if (t.has(`${sectionKey}.commonMistakes`)) {
        const mistakes = t.raw(`${sectionKey}.commonMistakes`) as Array<{ problem: string; solution: string }>;
        content.push(
      ...
      if (t.has(`${sectionKey}.troubleshooting`)) {
        const troubleshooting = t.raw(`${sectionKey}.troubleshooting`) as Array<{ problem: string; solution: string }>;
        content.push(
      ...
      if (t.has(`${sectionKey}.tips`)) {
        const tips = t.raw(`${sectionKey}.tips`) as string[];
        content.push(
      ...
      if (sectionId === 'creatingModules') {
        ...
        if (t.has(`${sectionKey}.aiTutorConfig`)) {
          const aiConfig = t.raw(`${sectionKey}.aiTutorConfig`) as { whatIs: string; howToWrite: string; example: string };
          content.push(
      ...
      if (sectionId === 'uploadingFiles') {
        ...
        if (t.has(`${sectionKey}.supportedFormats`)) {
          const formats = t.raw(`${sectionKey}.supportedFormats`) as { title: string; list: string[] };
          content.push(
      ...
      if (sectionId === 'creatingTokens') {
        ...
        if (t.has(`${sectionKey}.distribution`)) {
          const distribution = t.raw(`${sectionKey}.distribution`) as { title: string; methods: Array<{ name: string; description: string }> };
          content.push(
      ...
      if (sectionId === 'preparingModules') {
        ...
        if (t.has(`${sectionKey}.testing`)) {
          const testing = t.raw(`${sectionKey}.testing`) as { title: string; steps: Array<{ number: number; action: string }> };
          content.push(
        ...
        if (t.has(`${sectionKey}.finalChecklist`)) {
          const checklist = t.raw(`${sectionKey}.finalChecklist`) as { title: string; items: string[] };
          content.push(
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <ol className="ml-8 space-y-2 list-decimal">
                  {(t.raw('common.passwordSetup.steps') as string[]).map((step, idx) => (
                ...
                <div className="ml-4 space-y-4">
                  <div>
                    ...
                    <ul className="ml-4 space-y-1">
                      {(t.raw('common.profileSettings.theme.options') as string[]).map((option, idx) => (
                  ...
                  <div>
                    ...
                    <ul className="ml-4 space-y-1">
                      {(t.raw('common.profileSettings.language.options') as string[]).map((option, idx) => (
                        <li key={idx} className="text-sm flex items-start">
                          <ChevronRight className="h-4 w-4 mr-1 mt-0.5 text-primary shrink-0" />
                          <span>{option}</span>
                        </li>
